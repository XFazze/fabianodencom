...
fn site(site: &Website) -> Html {
    html! {
        <div class={classes!("flex","flex-col", "gap-1")}>
                <p class={classes!("text-2xl")}>{site.name.to_string()}</p>
                <a class={classes!("text-emerald-500", "text-sm")} href={format!("{}{}", "https://".to_string(), site.url.to_string())} target={"_blank"}>{"https://"}{site.url.to_string()}</a>
                <p class={classes!("text-base", "mt-4")}>{site.description.to_string()}</p>
...
pub fn home() -> Html {
    let websites = vec![Website {
        ...
        name: "League Log(in progress)".to_string(),
    ...
    html! {
        <>
            <Base  active_page="home">
                <div class={classes!("flex", "flex-col", "gap-12")}>
